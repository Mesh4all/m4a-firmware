...
REPOBASE="$(realpath "$( cd "$(dirname "${0}")" && pwd )/../../../")"
RELEASE="${1}"

if [ $# -lt 1 ]; then
    echo "usage: ${0} <release> [<old> [<repobase>]]" >&2
    exit 1
fi
...
START=$(git -C "${REPOBASE}" log -n 1 --pretty="format:%at" "${OLD}")
END=$(git -C "${REPOBASE}" log -n 1 --pretty="format:%at" "${RELEASE}")

...
static struct node *node_generate(struct node *n, const struct world *w, unsigned range) {
...
static void world_gen(struct world *w, unsigned num_nodes, unsigned width, unsigned height,
...
static void _print_nodes(struct node *nodes, unsigned num, unsigned color) {
    for (unsigned i = 0; i < num; ++i) {
        printf("# %s\t%d\t%d\t%u\t0x%x\n", nodes[i].name, nodes[i].x, nodes[i].y, nodes[i].r,
               _color(&nodes[i], color));
    }
}
...
static void get_random_mac(mac_t *mac) {
    for (uint8_t i = 0; i < MAC_BYTE_SIZE; i++) {
        mac->mac._u8[i] = (uint8_t)(rand() % 255);
    }
}

static void printf_random_macs(struct node *nodes, unsigned int numof_nodes) {
    for (unsigned i = 0; i < numof_nodes; ++i) {
        struct node *n = &nodes[i];
        get_random_mac(&n->mac);
        printf("%s := ", n->name);
        for (uint8_t j = 0; j < MAC_BYTE_SIZE; j++) {
            printf("%02X", n->mac.mac._u8[j]);
            if (j < MAC_BYTE_SIZE - 1) {
                printf(":");
            }
        }
        printf("\n");
...
int main(int argc, char **argv) {
    ...
    puts("# MAC Addresses");
    printf_random_macs(w.nodes, w.num_nodes);
    puts("");
    printf("# seed = %u\n", seed);
    puts("# Connections");

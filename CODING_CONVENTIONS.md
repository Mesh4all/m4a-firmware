...
* Please obey the Linux coding style as described in
  ...
  following modifications and additions:
    ...
    * There must be no trailing whitespace in any line.
      The script `/dist/tools/whitespacecheck/check.sh main || exit` can be
      used to detect whitespaces at the end of line(s) that would lead to build error(s).
...
* You can use [uncrustify](http://uncrustify.sourceforge.net/) with the provided
  option files: https://github.com/Mesh4all/m4a-firmware/blob/main/uncrustify-riot.cfg
...
* use `__restrict` instead of `restrict` in headers
...
* All files should contain the copyright note.
...
```
/*
 * Copyright (C) Copyright_year Mesh4all <contact@mesh4all.org>
 *
